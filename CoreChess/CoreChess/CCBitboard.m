...
const CCBitboard EmptyBB = 0;
const CCBitboard notAFile = 0xfefefefefefefefeULL;
const CCBitboard notHFile = 0x7f7f7f7f7f7f7f7fULL;
const CCBitboard longDiag = 0x8040201008040201ULL; // Long diagonal a1-h8
const CCBitboard longAdiag =0x0102040810204080ULL; // Long antidiagonal a8-h1

#pragma mark -

CCBitboard generate_ray(CCSquare s, CCDirection d)
{
    ...
    for (int i = 0; i < 7; i++) {
        rank += rankDir;
        file += fileDir;
        b = b | CCBitboardForSquare(CCSquareMake(rank, file));
    }
    
    return b;
}

CCSquare CCSquareForBitboard(CCBitboard b)
{
    if (b == 0)    return InvalidSquare; // Invalid square because bitboard is empty
    if (b & (b-1)) return InvalidSquare; // Invalid square because bitboard is overpopulated
...
int CCBitboardPopulationCount(CCBitboard b) {
    int count = 0;
...
CCBitboard CCBitboardMovesForPieceAtSquare(CCPiece p, CCSquare s)
{
    if (!CCSquareIsValid(s))
        return EmptyBB;
    
    if (CCPieceIsValid(p) && !CCPieceEqualToPiece(p, PawnPiece))
    {
        if      (p == KnightPiece) { return PieceMovesBB[0][(char)s];}
        else if (p == BishopPiece) { return PieceMovesBB[1][(char)s];}
        else if (p == RookPiece  ) { return PieceMovesBB[2][(char)s];}
        else if (p == QueenPiece ) { return PieceMovesBB[3][(char)s];}
        else if (p == KingPiece  ) { return PieceMovesBB[4][(char)s];}
    }
...
CCBitboard CCBitboardGetRankAttacks(CCBitboard occupied, CCSquare s) {
    unsigned char rank = CCSquareRank(s);
    CCBitboard board = occupied & RanksBB[rank]; // Mask the rank with occupied
    board = board >> (rank * 8);               // Translate to first rank
    board = occupancy_bitboards[CCSquareFile(s)][board];  // Get occupancy of rank from the file of s      
    return board << (rank * 8);                // Translate back to rankOf(s)
}

CCBitboard CCBitboardGetFileAttacks(CCBitboard occupied, CCSquare s) {
    unsigned char file = CCSquareFile(s);
    CCBitboard board = occupied & FilesBB[file]; // Mask the file with occupied
    CCBitboard adiag = 0x0102040810204080ULL;// Long antidiagonal h1-a8
    ...
    board = occupancy_bitboards[CCSquareRank(s)][board]; // Get occupancy
...
CCBitboard CCBitboardGetDiagonalAttacks(CCBitboard occupied, CCSquare s) {
    unsigned char rank = CCSquareRank(s);
    unsigned char file = CCSquareFile(s);  // Also serves as index
    
    CCBitboard diag = longDiag; // Long diagonal a1-h8
    ...
    CCBitboard board = (occupied & diag) * FilesBB[0];  // Mask the diagonal with occupied and 
...
CCBitboard CCBitboardGetAntiDiagonalAttacks(CCBitboard occupied, CCSquare s) {
    occupied |= CCBitboardForSquare(s);
    unsigned char rank = CCSquareRank(s);
    unsigned char file = 7 - CCSquareFile(s);
    
    CCBitboard adiag = longAdiag; // Long antidiagonal h1-a8
    ...
    CCBitboard board = (occupied & adiag);                 // Mask the diagonal with occupied and 
    board *= FilesBB[0];                                 // Multiply to get bits on 8th rank
    board = occupancy_bitboards[CCSquareFile(s)][board >> 56];  // Get occupancy
    board *= FilesBB[0];                                 // Fill each byte of the bitboard with occupancy
...
CCBitboard CCBitboardNorthOne(CCBitboard b) 
{
    return b << 8;
}

CCBitboard CCBitboardSouthOne(CCBitboard b)
{
    return b >> 8;
}

CCBitboard CCBitboardEastOne(CCBitboard b)
{
    return (b << 1) & notAFile;
}

CCBitboard CCBitboardNorthEastOne(CCBitboard b)
{
    return (b << 9) & notAFile;
}

CCBitboard CCBitboardSouthEastOne(CCBitboard b) 
{
    return (b >> 7) & notAFile;
}

CCBitboard CCBitboardWestOne(CCBitboard b)
{
    return (b >> 1) & notHFile;
}

CCBitboard CCBitboardSouthWestOne(CCBitboard b)
{
    return (b >> 9) & notHFile;
}

CCBitboard CCBitboardNorthWestOne(CCBitboard b)
{
    return (b << 7) & notHFile;
}

#pragma mark - Files / Ranks

CCBitboard filesBB(unsigned char file)
{
    return FilesBB[file]; 
}

CCBitboard ranksBB(unsigned char rank) 
{
    return RanksBB[rank];
}
...
const CCBitboard PieceMovesBB[5][64] = {
    // Knight moves
    {
...
void print_hex(const CCBitboard bb) {
    printf("0x%016llXULL,\n", bb);
}

void print_bitboard(CCBitboard bb)
{
    for (int i = 7; i >= 0 ; i--) {
        for (int j = 0; j < 8; j++) {
...
NSString * NSStringFromCCBitboard(CCBitboard bitboard)
{
